...
def run(config_filename):
    ...
    if save_status:
        ...
        if not os.path.exists(outdir):
            ...
            os.makedirs(outdir)
        #Two files to be opened (fieldname and fieldid given in the input yaml file)
        #One containing a summary of the simulation: astropy table with (SNID,Ra,Dec,X1,Color,z) parameters
        ...
        simu_out=outdir+'/Simu_'+fieldname+'_'+str(fieldid)+'_'+str(season_obs)+'.hdf5'
        lc_out=outdir+'/LC_'+fieldname+'_'+str(fieldid)+'_'+str(season_obs)+'.hdf5'
        #and these files will be removed now (before processing) if they exist (to avoid confusions)
