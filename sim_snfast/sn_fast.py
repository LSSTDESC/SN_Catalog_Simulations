...
class SN(SN_Object):
    ...
    def __init__(self, param, simu_param):
        ...
        lc_ref_tot = Load_Reference(
            simu_param['Reference File'], np.unique(zvals)).tab
    ...
    def __call__(self, obs, index_hdf5, display=False, time_display=0., gen_par=None):
        ...
        for j, band in enumerate(bands):
            ...
            p = multiprocessing.Process(name='Subprocess-'+str(
                j), target=self.Process_band, args=(obs[idx], band, gen_par, j, result_queue))
            p.start()
        ...
        #tab_tot = Table()
        tab_tot = [resultdict[j]
                   for j, band in enumerate(bands) if resultdict[j] is not None]
        ...
        if display:
            ...
            for DayMax in np.unique(sel_tab['DayMax']):
                ...
                self.Plot_LC(sel_tab[idxb]['time', 'band', 'flux', 'fluxerr',
                                           'zp', 'zpsys'], time_display, zref, DayMax, season)
    ...
    def Process_band(self, sel_obs, band, gen_par, j=-1, output_q=None):
        ...
        obs_time = np.ma.array(
            np.tile(sel_obs[self.mjdCol], (len(mag_obs), 1)), mask=~flag)
        seasons = np.ma.array(
            np.tile(sel_obs[self.seasonCol], (len(mag_obs), 1)), mask=~flag)
        ...
        tab.add_column(Column(self.mag_to_flux_e_sec[band](
    ...
    def Ana_LC(self, tab):
        ...
        for season in np.unique(tab['season']):
            ...
            for z in np.unique(sela['z']):
                if z > 0.7:
                    continue
            # for z in np.arange(0.05,0.64,0.05):
                time_refa = time.time()
                idxb = np.abs(sela['z']-z) < 1.e-5
                selb = sela[idxb]
                print('redshift', z, len(selb))
